...
internal class IslandWrackAccess(
    private val position: IslandPosition, private val config: ConfigurationHolder<WrackConfiguration>,
) : WrackAccess, KoinComponent {
    ...
    override suspend fun wrackTick(time: Long, level: Int, enchantments: EnchantmentHolder) {
        tickingWrackEntities.removeIf { !it.tick(time) }

        if (time >= nextSpawnTime && wrackEntities.size <= config.value().spawnLimit) {
    ...
    private fun spawnTick(time: Long, level: Int, enchantments: EnchantmentHolder) {
        ...
        nextSpawnTime = time + config.value().spawnIntervalTicks.sample(random)
    }
